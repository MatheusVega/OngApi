...
namespace AM.Amil.PeNaAreia.Api.Extensions
{
    static class ExceptionExtensions
    {
        public static string ToLogString(this Exception exception, string environmentStackTrace)
        {
            ...
            //Apenas enquanto estamos desenvolvendo
            //return $" {GetDescriptionException(exception.GetType().Name)} {environmentStackTraceLines.First().Split("//").Last().TrimStart()}";

            string retorno = $"{JsonConvert.SerializeObject(exception, Formatting.Indented)}. environmentStackTrace: {environmentStackTrace}";
            if(exception.InnerException != null)
            {
                retorno += $" - {JsonConvert.SerializeObject(exception, Formatting.Indented)}";
            }
